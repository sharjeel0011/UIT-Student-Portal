...
const TextBooks = () => {
  const [selectedBook, setSelectedBook] = useState(null);
  const [activeTab, setActiveTab] = useState(0); // Track active tab (0 = First Quarter)

  const textbooksByQuarter = [
    {
    ...
    // Add more quarters and books as needed
  ];
  ...
  return (
    <div className="flex min-h-screen bg-gray-100">
      ...
      <div className="p-16 bg-gray-100 min-h-screen w-full">
        <h1 className="text-2xl font-bold text-blue-900 mb-6">TEXT BOOKS</h1>

        {/* Tabs */}
        <div className="flex border-b mb-6">
          {textbooksByQuarter.map((quarter, index) => (
            <button
              key={index}
        ...
        <div className="space-y-4">
          {textbooksByQuarter[activeTab]?.books.map((textbook) => (
            <div
              key={textbook.id}
              className="bg-white shadow-md rounded-lg p-4 border border-gray-300"
            >
